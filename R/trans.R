...
# Check if function body input was done correctly:
	
if(class(as.numeric(colnames(raw)))!="numeric"){stop("Invalid argument: the colnames of 'raw', which should be the waveband positions, are not coercible to class 'numeric'.")}
...
if(class(raw)=="data.frame"){raw<-as.matrix(raw)};
if(class(raw)!="matrix"){stop("Invalid argument: 'raw' must be of class 'matrix' or 'data.frame'.")};
if(is.na(match(tr,c("derivative","continuum removed","wavelet transformed")))){stop("Invalid argument: 'tr' must be 'derivative','continuum removed' or 'wavelet transformed'")}
if(tr=="derivative"){order<-round(order);if(is.na(match(order,c(0:3)))){stop("Invalid argument: 'order' has to be an integer between 0 and 3.")};gap<-round(gap);if(is.na(match(gap,c(1:30)))){stop("Invalid argument: 'gap' has be an integer between 1 and 30.")}};

# Derivative:

if(tr=="derivative"){
trans<-matrix(nrow=nrow(raw),ncol=ncol(raw),dimnames=list(rownames(raw),colnames(raw)));
waveb<-as.numeric(colnames(raw));
require(KernSmooth,quietly=T);
for(i in 1:nrow(raw)){
	trans[i,]<-locpoly(waveb,raw[i,],drv=order,bandwidth=gap,gridsize=ncol(raw))[[2]]
	}
...
raw.comp<-matrix(nrow=nrow(raw),ncol=length(waveb.1024),dimnames=list(rownames(raw),waveb.1024));
for(i in 1:nrow(raw)){
	raw.comp[i,]<-round(spline(waveb,raw[i,],method="natural",xout=waveb.1024)[[2]],6);
}
library(wavelets);
lev<-7;
slo<-3;
filte="haar";
trans<-matrix(nrow=nrow(raw.comp),ncol=2^lev,dimnames=list(rownames(raw.comp),paste("WC_",c(1:2^lev),sep="")));
